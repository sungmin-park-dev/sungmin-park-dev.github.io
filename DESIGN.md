...
`$color-text-primary`(거의 순수한 검정/흰색)는 실제로 읽는 콘텐츠 전용이다 —
프로젝트 본문(`.post-body`, 코드 블록 포함)과 About의 `.section-content`
문단. 이 텍스트는 사용자가 실제로 정독하는 대상이므로 회색으로 흐려서는 안
된다.

`$color-text-secondary`/`$color-text-tertiary`(반투명 회색)는 콘텐츠가 아니라
메타데이터·라벨·"아직 활성화되지 않음"을 나타낼 때만 쓴다 — 날짜, 태그,
브레드크럼, 인덱스 카드의 부제, TOC에서 아직 스크롤이 닿지 않은 항목
(`.toc-link`, 활성화되면 primary-blue로 전환) 등. 회색 자체가 "지금 이건
본문이 아니거나 아직 대상이 아니다"라는 뜻이 되도록 통일한다.
